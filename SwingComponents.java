...
class SwingComponents extends JFrame implements ActionListener {
    ...
    SwingComponents() {
        // Setting up JFrame and Container
        c = getContentPane();
        ...
        jb1.setFont(f);
        jb1.setSelectedIndex(2); // Preselect "Java"
        c.add(jb1);
        ...
        ajp.setSelected(true); // AJP selected by default
        cns.setEnabled(false); // CNS disabled
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == b1) {
            l6.setVisible(true); // Show message label when button is clicked
        }
    }

    public static void main(String args[]) {
        // Wrap GUI creation in SwingUtilities.invokeLater to ensure it's created on the EDT
        SwingUtilities.invokeLater(() -> new SwingComponents());
    }
}
